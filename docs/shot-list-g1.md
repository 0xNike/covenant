...
console       http://localhost:3007/transactions, "block A, issuance" tab
```
...
lives at `/transactions`, under the "block A, issuance" tab — it is not the default tab on that
...
connected wallet at all). go to `http://localhost:3007/transactions`, then click the "block A,
