...
| F3 | **documentation gap, upstream PR candidate. rudolph to assess.** ATS `scheduledTask` reads as Hedera Schedule Service integration and is not. it is an internal EVM task queue in `domain/orchestrator/ScheduledTasksOps.sol`, drained lazily by the next state-mutating call. six greps for `IHederaScheduleService`, `ScheduleCreate`, `scheduleCreate`, `HederaScheduleService`, `0x16b` and the padded system contract address return **zero hits** across `packages/ats/contracts`. the sdk's whole `port/in/scheduledTask` surface is two read queries (`ScheduledCouponListing.ts:27,38`), with no write call to trigger a scheduled coupon. nothing in the docs says so, and the name actively implies otherwise. **we were about to claim a prize line on this misreading, which is the strongest possible evidence that the naming misleads.** a docs PR clarifying the distinction is a genuine contribution and costs no Solidity | ATS contracts + sdk v8.0.0 | open, candidate |
